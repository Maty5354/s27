...
(function() {
    ...
    const scheduleData = {
        ...
        1: [ // Monday
            { time: '12:00', subject: 'Chimie', emoji: '🧪' },
            { time: '13:00', subject: 'Mate', emoji: '📏' },
            { time: '14:00', subject: 'Istorie', emoji: '📜' },
            { time: '15:00', subject: 'Civică', emoji: '🏛️' },
            { time: '16:00', subject: 'Sport', emoji: '⚽' },
            { time: '17:00', subject: 'Franceză', emoji: '<span class="fi fi-fr" style="border-radius: var(--border-radius-sm);"></span>' }
        ],
        2: [ // Tuesday
            { time: '13:00', subject: 'Informatică', emoji: '💻' },
            { time: '14:00', subject: 'Mate', emoji: '📏' },
            { time: '15:00', subject: 'Opt Mate', emoji: '📐' },
            { time: '16:00', subject: 'Fizică', emoji: '💡' },
            { time: '17:00', subject: 'Franceză', emoji: '<span class="fi fi-fr" style="border-radius: var(--border-radius-sm);"></span>' },
            { time: '18:00', subject: 'Sport', emoji: '⚽' }
        ],
        3: [ // Wednesday
            { time: '12:00', subject: 'Geografie', emoji: '🌍' },
            { time: '13:00', subject: 'Mate', emoji: '📏' },
            { time: '14:00', subject: 'Tehnologică', emoji: '🧰' },
            { time: '15:00', subject: 'Engleză', emoji: '<span class="fi fi-gb" style="border-radius: var(--border-radius-sm);"></span>' },
            { time: '16:00', subject: 'Istorie', emoji: '📜' },
            { time: '17:00', subject: 'Română', emoji: '<span class="fi fi-ro" style="border-radius: var(--border-radius-sm);"></span>' }
        ],
        4: [ // Thursday
            { time: '12:00', subject: 'Română', emoji: '<span class="fi fi-ro" style="border-radius: var(--border-radius-sm);"></span>' },
            { time: '13:00', subject: 'Muzică', emoji: '🎵' },
            { time: '14:00', subject: 'Mate', emoji: '📏' },
            { time: '15:00', subject: 'Chimie', emoji: '🧪' },
            { time: '16:00', subject: 'Engleză', emoji: '<span class="fi fi-gb" style="border-radius: var(--border-radius-sm);"></span>' },
            { time: '17:00', subject: 'Religie', emoji: '☦️' },
            { time: '18:00', subject: 'Română', emoji: '<span class="fi fi-ro" style="border-radius: var(--border-radius-sm);"></span>' }
        ],
        5: [ // Friday
            { time: '12:00', subject: 'Dirigenție', emoji: '📰' },
            { time: '13:00', subject: 'Fizică', emoji: '💡' },
            { time: '14:00', subject: 'Biologie', emoji: '🧬' },
            { time: '15:00', subject: 'Română', emoji: '<span class="fi fi-ro" style="border-radius: var(--border-radius-sm);"></span>' },
            { time: '16:00', subject: 'Geografie', emoji: '🌍' },
            { time: '17:00', subject: 'Arte', emoji: '🎨' }
        ]
    };
        ...
        todaySchedule.forEach((cls, index) => {
            ...
            card.addEventListener('click', () => {
                ...
                if (mode === 'mark') {
                    ...
                    const subject = cls.subject.replace(/<[^>]*>/g, '').toLowerCase();
                    window.open(`https://manuale.edu.ro/?s=${encodeURIComponent(subject)}`, '_blank');
                }
            });
