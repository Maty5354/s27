...
function fetchWeatherData(position) {
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            if (sheetEmoji) sheetEmoji.textContent = emoji;
            if (sheetTemp) sheetTemp.textContent = temp;
...
function updateWeatherError(message) {
    ...
    if (sheetEmoji) sheetEmoji.textContent = "❌";
    if (sheetTemp) sheetTemp.textContent = "Error";
...
if (sheetWeatherBtn) {
    sheetWeatherBtn.addEventListener("click", () => {
        if (window.overlayManager) {
            // Close bottom sheet if mobileNav exists
            if (window.mobileNav && window.mobileNav.closeBottomSheet) {
